...
#define FW_FILENAME "moxa/moxa-1130.fw"
...
struct mxu1_port {
	...
	int mxp_closing_wait;
	struct async_icount mxp_icount;
	wait_queue_head_t mxp_msr_wait;
...
static int mxu1_download_firmware(struct mxu1_device *mxdev,
		const unsigned char *firmware, unsigned int firmware_size);
	...
	if (dev->config->interface[0]->cur_altsetting->desc.bNumEndpoints == 1) {
		switch(mxdev->mxd_model_name){
			case MXU1_MODEL_1130:
				err = request_firmware(&fw_p, FW_FILENAME , &serial->interface->dev);

				if (err) {
					dev_warn(&serial->interface->dev, "Firmware " FW_FILENAME " not found\n");
					goto free_mxdev;
				}

				status = mxu1_download_firmware(mxdev, fw_p->data, fw_p->size);
				break;
			default:
				status = -EINVAL;
				break;
		...
		unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case MOXA_SET_INTERFACE:
			...
			if(mxport->mxp_mxdev->mxd_model_name != MXU1_MODEL_1110){ //UPort 1130
				switch(arg){
					case MXU1_RS232: //UPort 1150, 1150I
						...
						mxport->mxp_uart_mode = MXU1_UART_232;
						break;
					case MXU1_RS422: //UPort 1130
					case MXU1_RS4854W: //UPort 1130
						mxport->mxp_uart_mode = MXU1_UART_485_RECEIVER_ENABLED;
						break;

					case MXU1_RS4852W: //UPort 1130
						mxport->mxp_uart_mode = MXU1_UART_485_RECEIVER_DISABLED;
						break;
					default:
						return -EINVAL;
		...
		struct ktermios *old_termios)
{
	...
	if (old_termios)
		dbg("%s - old cflag %08x, old iflag %08x", __FUNCTION__, old_termios->c_cflag, old_termios->c_iflag);
